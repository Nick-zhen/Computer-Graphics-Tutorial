...
rightEye.position.set(0.6, 12, 3.3);
armadilloFrame.add(leftEye);
leftEye.position.set(-0.6, 12, 3.3);
// Laser Beams (Q1b Q1c)
...
function checkKeyboard() {
  ...
  if (keyboard.pressed("S")) sphere.translateZ(.1);
  if (keyboard.pressed("D")) sphere.translateX(.1);
  if (keyboard.pressed("W")) sphere.translateZ(-.1);
  if (keyboard.pressed("A")) sphere.translateX(-.1);
  if (keyboard.pressed("Q")) sphere.translateY(.1);
  if (keyboard.pressed("E")) sphere.translateY(-.1);
  if (keyboard.pressed("b")) armadilloFrame.translateY(-.1);
  ...
  var rightEyeWorldPos = new THREE.Vector3();
  rightEye.getWorldPosition(rightEyeWorldPos);
  var rightDistance = sphere.position.distanceTo(rightEyeWorldPos);

  var leftEyeWorldPos = new THREE.Vector3();
  leftEye.getWorldPosition(leftEyeWorldPos);
  var leftDistance = sphere.position.distanceTo(leftEyeWorldPos);
  ...
  rightLaser.scale.set(1,rightDistance*2,1);
  leftLaser.scale.set(1,leftDistance*2,1);

  rightLaser.rotation.x = Math.PI/2;
  leftLaser.rotation.x = Math.PI/2;

  rightLaser.position.z = rightDistance;
  leftLaser.position.z = leftDistance;
  
  if (rightDistance < LaserDistance) {
