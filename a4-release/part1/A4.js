...
// order left, right, top, bottom, behind, front
const skyboxTexture = skyboxCubemap.load( ['cube1.png', 'cube4.png', 
'cube2.png', 'cube5.png', 'cube3.png', 'cube6.png',
...
const shayDMaterial = new THREE.ShaderMaterial({
  ...
  uniforms: {
    ...
    // Qa Pass the uniforms for blinn-phong shading,colorMap, normalMap  to the shaderMaterial
...
function update() {
  ...
  else if (sceneHandler == 2) 
  {
    ...
    // First-pass: render scene and save render result in renderTarget.texture
    renderer.setRenderTarget(renderTarget);
    renderer.render(shadowScene, shadowCam);
    
    // Second-pass: apply a shadow effect to the previous render's output saved in renderTarget.texture
  ...
  else if (sceneHandler == 3) 
  {
    ...
    // Second-pass: apply a shadow effect to the previous render's output saved in renderTarget.texture
    floorMaterial.uniforms.textureSize.value = renderTarget.innerWidth;
...
renderer.getSize(screenSize);
floorMaterial.uniforms.textureSize.value = renderTarget.screenSize;
